  ...
  methods: {
    ...
    async step1xml(file) {
      try {
        ...
        const response = await http.post(`/Task/getFactura`, requestParams);
    ...
    verificar() {
      ...
      this.itemsFacturaProcesar.listbills.forEach((element) => {
        ...
        this.step2xml(element).then((res) => {
          ...
          if (res.status === "Ok") {
            this.knowledge =
              this.knowledge + parseFloat(parte.toFixed(2)) * res.data;
          ...
          console.log(
            "conatdor: " +
              contador +
              " / " +
              this.itemsFacturaProcesar.listbills.length
          );
          if (contador == this.itemsFacturaProcesar.listbills.length) {
            if (duplicado) {
              ...
              alert(
                "Los siguientes productos no estan registrados: " + listaError
              );
            } else {
            ...
            this.step2xmlSave(element).then((res) => {
              ...
              if (conta == this.itemsFacturaProcesar.listbills.length) {
                if (this.errorList.length == 0) {
                  var mensaje =
                    "La factura se cargo Exitosamente " +
                    conta +
                    " de " +
                    this.itemsFacturaProcesar.listbills.length;
                  alert(mensaje);
    ...
    async step2xml(Detallefactura) {
      try {
        ...
        const response = await http.post(`/Task/LoadProduct`, requestParams);
    ...
    step2xmlSave(Detallefactura) {
      try {
        ...
        const response = http.post(`/Task/SaveProduct`, requestParams);
    ...
    readFile() {
      if (this.file) {
        ...
        if (this.banderaxml) {
          ...
          fileReader.onload = (e) =>
            this.parseFilexml(utf8.decode(e.target.result));
          fileReader.onerror = () => alert("Error al cargar el archivo");
        }
      }
    },
    ...
    async uploadDataToServer(itemData, campaignId, optionId, statusId) {
      try {
        ...
        itemData.Encuesta = itemData.Encuesta.toString();
      } catch (error) {
        alert(
          "Error en los formatos del archivo refresque la paguina y vuelva a cargar"
        );
      }
