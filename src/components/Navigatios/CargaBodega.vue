<template>
  <div class="pb-15">
    ...
    <v-row justify="center">
      <v-dialog v-model="dialogRegOrder" persistent max-width="860px">
              <v-card>
                <v-card-title >
                  <v-col cols="12" sm="12" md="12" class="pt-0 pb-0" style="text-align: center;">
                    <span class="headline">Detalle del Devolución</span>
                  </v-col>
                </v-card-title>
                <v-card-text>
                  <v-container>
    ...
    <v-row>
      <v-col>
        <h1 class="tText">Stock Mercaderistas</h1>
      </v-col>
    ...
    <v-row class="justify-lg-space-between">
      <v-col lg="4" cols="sm" class="pb-2">
        <v-card class="rounded-xl pa-3" rounded="true">
          <v-row class="align-baseline justify-space-between">
            <v-col class="text-top pr-3" lg="12">
              <v-text-field
                v-model="searchQuery"
                dense
      ...
      <v-col lg="8" cols="sm" class="pb-2">
        <v-card class="rounded-xl pa-3" rounded="true">
          ...
          >
            <template v-slot:[`item.actions`]="{ item }">
              <v-icon small color="error" class="mr-4" @click="addOrder(item)">
                mdi-truck-fast
              </v-icon>
            </template>
            <!-- <template v-slot:[`item.actions`]="{ item }">
...
export default {
  ...
  methods: {
    ...
    addItemOrder(){
      ...
      if(this.cantidad_tranferir <= this.editedItem.cantidad){
      let indice = 0;
      ...
      console.log("Encontro un candidato")
      console.log(this.selectedRouteData.id.toString())
      this.pollstersItems.forEach(element => {
      ...
      this.dialogOrder = false;
        ...
        this.mostrarMesaje(true,"No existe stock sufieciente","red");
      }
    ...
    async sendOrder(){
      ...
      try {
        ...
        order: {idorder: 0,idaccount:this.getUserData.idAccount,description: "transferencia MD",status:"pendiente",observation:"",image:"",origin: this.selectedRouteData.id.toString(),destination:this.getUserData.idDistribuidor,},
        details:[]}
      ...
      console.log(JSON.stringify(orderModel));
        ...
        this.mostrarMesaje(true,'La devolución se genero Exitosamente','green');
        this.dialogRegOrder = false;
      } catch (dataError) {
        if ("status" in dataError && "data" in dataError) {
          this.errorList.push(dataError.data);
        this.mostrarMesaje(true,'La devolución NO se genero','red');
        }
      } finally {
        console.log("");
      }
    ...
    async getCampaignStatus() {
      try {
        ...
        const response = await http.post(`/Surti/ObtnerVenddoresActivos?tipo=2`, requestParams);
        if (response.status === "Ok") {
