...
export default {
  ...
  methods: {
    ...
    async duplicateValidate (codigo){
      var bandera = true;
      await this.getProducts();
      for (var i = 0, len = this.productItems.length; i < len; i++) {
          if(codigo == this.productItems[i].codigo){
            bandera = false;
            return bandera
          } 
      }
    ...
    async save() {
      ...
      var duplicated = await this.duplicateValidate(this.editedItem["codigo"] == null?"":this.editedItem["codigo"].toString())
      this.editedIndex == null?this.editedIndex = -1:this.editedIndex
      
       if(!duplicated && this.editedIndex <= -1){
        valid = false;
        this.textbar = 'El codigo del producto esta duplicado'
        this.snackbar = true;
      }
